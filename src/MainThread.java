...
//        int WindowHeight = Window.Height("CabinetWClass","计算机");
        ...
        Main.jl1.setText("鼠标所在窗口的宽为："+WindowWidth+"高为："+WindowHeight);
        //通过java自带的robot来模拟操作
        ...
        while (true){
            ...
            if(Main.x>0 && Main.y>0){
                if(robot!=null){
                    Main.jl1.setText("正在执行操作：左键单击");
                    //鼠标移动到图片的位置
                    //这里因为是用的程序的数据查早到的结果，所以在用的时候要加上程序相对于屏幕的坐标，如果您是用的全屏寻找，则不用进行这一步
                    robot.mouseMove(Main.x + WindowLeftUpXCoordinate, Main.y + WindowLeftUpYCoordinate);
                    //左键单击
                    ...
                    Main.jl1.setText("正在将鼠标移动到窗口的右下角");
                    robot.mouseMove(WindowLeftUpXCoordinate + WindowWidth,WindowLeftUpYCoordinate + WindowHeight);
                }
            }
            try {
                Main.jl1.setText("本次主线程操作已完成，休息一会");
                Thread.sleep(1000);
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
